...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--scheme-id", type=int, default=1, help="SKOSConceptScheme-ID")
        parser.add_argument(
            "--format",
            default="ttl",
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Following Error occured: {e}"))
            return None
        qs = SkosConcept.objects.filter(scheme=scheme)
        self.stdout.write(f"found {qs.count()} SkosConcepts for SkosConceptScheme >>{scheme}<< with ID: {scheme.id}")

        file_name = f"{file_name_part}.{export_format}"
        self.stdout.write(f"start writin to {file_name}")
