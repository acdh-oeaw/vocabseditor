	...
	for obj in results:
		...
		if obj.skos_related.all():
			for x in obj.skos_related.all():
				g.add((concept, SKOS.related, URIRef(x.scheme.indentifier + "#concept" + str(x.id))))
		if obj.related.all():
		...
		if obj.skos_broadmatch.all():
			for x in obj.skos_broadmatch.all():
				g.add((concept, SKOS.broadMatch, URIRef(x.scheme.indentifier + "#concept" + str(x.id))))
		if obj.narrowmatch.all():
			for x in obj.narrowmatch.all():
				g.add((concept, SKOS.narrowMatch, URIRef(x.scheme.indentifier + "#concept" + str(x.id))))
		...
		if obj.skos_exactmatch.all():
			for x in obj.skos_exactmatch.all():
				g.add((concept, SKOS.exactMatch, URIRef(x.scheme.indentifier + "#concept" + str(x.id))))
		if obj.exactmatch.all():
			for x in obj.exactmatch.all():
				g.add((concept, SKOS.exactMatch, URIRef(x.scheme.indentifier + "#concept" + str(x.id))))
		if obj.skos_relatedmatch.all():
			for x in obj.skos_relatedmatch.all():
				g.add((concept, SKOS.relatedMatch, URIRef(x.scheme.indentifier + "#concept" + str(x.id))))
		if obj.relatedmatch.all():
			for x in obj.relatedmatch.all():
				g.add((concept, SKOS.relatedMatch, URIRef(x.scheme.indentifier + "#concept" + str(x.id))))
		if obj.skos_closematch.all():
			for x in obj.skos_closematch.all():
				g.add((concept, SKOS.closeMatch, URIRef(x.scheme.indentifier + "#concept" + str(x.id))))
